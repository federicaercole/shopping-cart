...
function QuantityInput({ product, input, defaultValue, onBlur, onChange, handleButtons }) {
    ...
    return (
        <>
            ...
            <input type="number" id={input} defaultValue={defaultValue} min="1" max={product.quantity} step="1" onChange={onChange} onBlur={onBlur} aria-invalid="false" aria-describedby={`error-${product.url}`} required />
